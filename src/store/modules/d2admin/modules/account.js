...
import { loginApi } from '@/api/auth/permApi.js'
...
export default {
  ...
  actions: {
    ...
    async login({ dispatch }, {
      ...
      const res = await mixins.methods.handleHttpMethod(loginApi(params), false)
      // 设置 cookie 一定要存 uuid 和 token 两个 cookie
      // 整个系统依赖这两个数据进行校验和存储
      // uuid 是用户身份唯一标识 用户注册的时候确定 并且不可改变 不可重复
      // token 代表用户当前登录状态 建议在网络请求中携带 token
      // 如有必要 token 需要定时更新，默认保存一天
      util.cookies.set('uuid', res.data.guid)
      util.cookies.set('token', res.data.tokenId)
      // 设置 vuex 用户信息
